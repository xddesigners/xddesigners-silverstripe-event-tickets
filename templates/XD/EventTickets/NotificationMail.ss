...
<head>
    ...
    <title><%t XD\EventTickets\Model\Reservation.NotificationSubject 'New reservation for {event} by {name}' event=$TicketPage.EventTitle name=$Name %></title>
    <style type="text/css">
        * {
...
<body itemscope itemtype="http://schema.org/EmailMessage">
    ...
    <p><%t XD\EventTickets\Model\Reservation.NotificationSubject 'New reservation for {event} by {name}' event=$TicketPage.EventTitle name=$Name %></p>

    <table class="order-content" width="100%" valign="baseline">
        <tr valign="baseline">
            <th class="align-left" valign="baseline" align="left"><%t XD\EventTickets\Fields\SummaryField.Ticket 'Ticket' %></th>
            <% with $Attendees.First %>
                <% loop $TableFields %>
                    <th class="align-left" valign="baseline" align="left">$Header</th>
                <% end_loop %>
            <% end_with %>
            <th class="align-right" valign="baseline" align="right"><%t XD\EventTickets\Fields\SummaryField.Price 'Price' %></th>
        </tr>
        <% loop $Attendees %>
            <tr valign="baseline">
        ...
        <tr valign="baseline" class="divider">
            <td class="align-left" colspan="{$Top.Attendees.First.TableFields.Count}" valign="baseline" align="left">
                <strong><%t XD\EventTickets\Fields\SummaryField.Total 'Total' %></strong>
            </td>
            <td class="align-left" valign="baseline" align="left">&nbsp;</td>
